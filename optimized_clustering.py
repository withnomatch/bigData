...
def evaluate_clustering(predictions):
    ...
    print("Silhouette Score: %.4f" % silhouette_score)
    ...
    return silhouette_score, quality_score, avg_cluster_size, std_size
...
def run_single_experiment(df, params, k=50):
    ...
    # 评估
    silhouette_score, quality_score, avg_size, std_size = evaluate_clustering(predictions)
...
def save_comparison_results(results, output_path):
    ...
    print("=" * 80)
    print("%-15s %-10s %-10s %-12s %-12s %-12s %-12s %-12s" % (
        "vocabSize", "minDF", "maxDF", "vocab", "time", "silhouette", "quality", "avg_size"
    ))
    print("-" * 80)
    
    for result in results:
        params = result['params']
        print("%-15d %-10d %-10.2f %-12d %-12.2f %-12.4f %-12.4f %-12.2f" % (
            params['max_features'],
            params['min_df'],
            params['max_df'],
            result['vocab_size'],
    ...
    print("Silhouette Score: %.4f" % best_result['silhouette_score'])
