...
完整实验已在 node5 的 Spark 2.0 集群上完成。实验读取总记录数 152758，10% 抽样后有效记录 15441；TF-IDF 特征构建与缓存耗时约 1124.79 秒，7 组 K-Means 参数实验完成后总耗时约 1606.29 秒。结果已写入 `hdfs://10.176.62.230:9000/user/root/output/f_param_optimization`，本地小结果文件为 `f_experiment_results.jsonl` 和 `f_best_config.jsonl`。

## 完整实验结果

| 实验 | 初始化 | 距离/特征 | maxIter | seed | 训练时间 | 近似轮廓 | 最大簇占比 | 单点簇 |
| --- | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: |
| F01 | random | cosine / L2 TF-IDF | 20 | 42 | 16.76s | 0.1211 | 19.72% | 0 |
| F02 | k-means\|\| | cosine / L2 TF-IDF | 20 | 42 | 15.05s | 0.1149 | 37.31% | 1 |
| F03 | k-means\|\| | euclidean / raw TF-IDF | 20 | 42 | 165.42s | 0.7320 | 99.02% | 36 |
| F04 | k-means\|\| | cosine / L2 TF-IDF | 10 | 42 | 9.72s | 0.1122 | 40.08% | 1 |
| F05 | k-means\|\| | cosine / L2 TF-IDF | 30 | 42 | 21.93s | 0.1151 | 36.42% | 1 |
| F06 | k-means\|\| | cosine / L2 TF-IDF | 20 | 2026 | 15.09s | 0.1183 | 35.50% | 1 |
| F07 | k-means\|\| | cosine / L2 TF-IDF | 20 | 3407 | 18.17s | 0.1196 | 30.35% | 0 |

脚本按近似轮廓系数优先选择的 `best_config` 是 F03，但 F03 的最大簇占比达到 99.02%，且出现 36 个单点簇，说明原始 TF-IDF 欧氏距离在该数据上形成了退化聚类，不适合作为面向问题去重的最终方案。

综合近似轮廓、最大簇占比、单点簇数量和运行时间，建议最终采用 F01：`random + cosine + maxIter=20`。该方案在余弦等价方案中近似轮廓最高，K-Means 训练耗时 16.76 秒，最大簇占比 19.72%，没有单点簇，簇分布更稳定。
...
完整实验表明，单看近似轮廓系数会选到 F03，但该配置产生 99.02% 的超大簇，属于指标陷阱。最终建议采用 F01：`random + cosine + maxIter=20`，因为它在质量、簇分布和运行时间之间更均衡。`maxIter=30` 相比 `20` 提升很小但耗时更高，因此没有必要继续加大迭代次数。
